...
def get_senate_bill_soup(c, b, t):
    url = 'https://www.congress.gov/bill/{}th-congress/senate-bill/{}/{}'.format(c, b, t)
    soup = None
    tries = 0
...
def fetch_bill(c, b):
    ...
    root = LEP.get_bill_root(c, b)
    if not path.exists(root):
        os.makedirs(root)

    # fetch actions tab and header
    t = LEP.tabs[0]
    soup = get_senate_bill_soup(c, b, t)
    save_page(soup.select('.featured')[0], LEP.get_bill_tab(c, b, 'header'))
    logging.debug('got {} {}:{}'.format('header', c, b))
    save_page(soup.select('#main')[0], LEP.get_bill_tab(c, b, t))
    ...
    for t in LEP.tabs[1:]:
        ...
        if len(eles) > 0:
            save_page(eles[0], LEP.get_bill_tab(c, b, t))
            logging.debug('got {} {}:{}'.format(t, c, b))
        else:
            save_page(soup, LEP.get_bill_tab(c, b, t))
